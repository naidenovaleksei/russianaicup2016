...
class BattleFront:
    ...
    def calc_front_destination(self, line_type: LineType, me: Wizard):
        ...
        path_length = self.path_length[line_type]
        score = 0
        friend_sum_pos = me.x + (self.bottom - me.y)
        friend_sum_life = me.life
        enemy_sum_life = enemy_sum_pos = 0
        for unit in self.line_fronts[line_type]:
            position = unit.x + (self.bottom - unit.y)
            life = unit.life
        ...
        return (common_pos - (me.x + (self.bottom - me.y))) / path_length
